...
if __name__ == '__main__':
    ...
    # Get the logger
    util.toolbox.set_logger(os.path.join(args.model_dir, 'evaluate.log'))
    ...
    # Define the model
    model = u_net.UNet(params).cuda() if params.cuda else u_net.UNet(params)
    ...
    util.toolbox.load_checkpoint(
        os.path.join(args.model_dir, f'{args.restore_file}.pth.tar'), model
    )

    # Evaluate
    test_metrics = evaluate(model, loss_fn, test_dl, metrics, params)
    save_path = os.path.join(
        args.model_dir, f"metrics_test_{args.restore_file}.json"
    )
    util.toolbox.save_dict_to_json(test_metrics, save_path)